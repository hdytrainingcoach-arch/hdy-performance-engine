...
export default function Workspace(){
 ...
 const [players,setPlayers]=useState<Row[]>([]),[tests,setTests]=useState<Row[]>([]),[questionnaires,setQuestionnaires]=useState<Row[]>([]),[pain,setPain]=useState<Row[]>([]),[rpe,setRpe]=useState<Row[]>([]);
 ...
 async function load(){const q=org===DIAMBARS?supabase.from('players').select('id').eq('organization_id',org).eq('team_id',team).eq('active',true):supabase.from('players').select('id').eq('organization_id',org).eq('active',true);const {data:p}=await q;const list=(p||[]) as Row[];setPlayers(list);const ids=list.map(x=>x.id);if(!ids.length){setTests([]);setQuestionnaires([]);setPain([]);setRpe([]);return}const [t,h,pa,r]=await Promise.all([supabase.from('test_results').select('id,player_id,tested_at').in('player_id',ids).limit(1000),supabase.from('questionnaire_responses').select('id,player_id,submitted_at').eq('organization_id',org).in('player_id',ids).limit(1000),supabase.from('pain_declarations').select('id,player_id,created_at').eq('organization_id',org).in('player_id',ids).limit(1000),supabase.from('session_rpe').select('id,player_id,submitted_at').eq('organization_id',org).in('player_id',ids).limit(1000)]);setTests((t.data||[]) as Row[]);setQuestionnaires((h.data||[]) as Row[]);setPain((pa.data||[]) as Row[]);setRpe((r.data||[]) as Row[])}
 ...
 const painPlayers=useMemo(()=>new Set(pain.map(x=>x.player_id)).size,[pain]);
 ...
 <section style={S.metrics}><Metric label='ATHLÈTES ACTIFS' value={players.length}/><Metric label='HOOPER MANQUANTS' value={missingHooper}/><Metric label='DOULEURS SIGNALÉES' value={painPlayers}/><Metric label='TESTS ENREGISTRÉS' value={tests.length}/><Metric label='RPE ENREGISTRÉS' value={rpe.length}/></section>
