...
export default function HDYInstall(){
  ...
  return <main style={S.main}>
    <div aria-hidden='true' style={S.art}/><div aria-hidden='true' style={S.fade}/>
...
const S:Record<string,React.CSSProperties>={
  ...
  art:{position:'absolute',inset:0,backgroundImage:`url('/pwa/hdy-splash.webp?v=${V}')`,backgroundSize:'cover',backgroundPosition:'center center',backgroundRepeat:'no-repeat'},
  fade:{position:'absolute',inset:0,background:'linear-gradient(180deg,transparent 0%,transparent 68%,rgba(0,0,0,.5) 84%,#020202 100%)',pointerEvents:'none'},
  dock:{position:'absolute',zIndex:2,left:16,right:16,bottom:'max(14px,env(safe-area-inset-bottom))',maxWidth:520,margin:'0 auto',display:'grid',gap:9,padding:'12px',borderRadius:22,background:'rgba(10,10,12,.58)',border:'1px solid rgba(255,255,255,.09)',boxShadow:'0 18px 55px rgba(0,0,0,.42)',backdropFilter:'blur(18px)',WebkitBackdropFilter:'blur(18px)'},
